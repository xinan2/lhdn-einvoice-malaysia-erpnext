...
def salesinvoice_data(invoice,invoice_number):
            try:
                ...
                cbc_ID.text = str(sales_invoice_doc.name)  # Erp sales invoice name
...
def company_Data(invoice,sales_invoice_doc): #supplier data
            try:
                ...
                cbc_ID_2.text =company_doc.tax_id   
                ...
                cbc_ID_Identification = ET.SubElement(cac_PartyIdentification, "cbc:ID")
                ...
                cbc_ID_Identification.text = supplier_id_number

                # Supplier’s SST Registration Number
                if company_doc.custom_sst_registration_no:
                    cbc_ID_SST = ET.SubElement(cac_PartyIdentification, "cbc:ID")
                ...
                if company_doc.custom_tourism_tax_registration:
                    cbc_ID_TTX = ET.SubElement(cac_PartyIdentification, "cbc:ID")
                ...
                if company_doc.custom_contact_no:
                    cac_Contact = ET.SubElement(cac_Party_1, "cac:Contact")
                ...
                # Supplier’s Business Activity Description (MISC description)
                ...
                for address in address_list:
                    ...
                    cbc_CityName.text = address.city if address.city else "NA"

                    cbc_PostalZone = ET.SubElement(cac_PostalAddress, "cbc:PostalZone")
                    cbc_PostalZone.text = address.pincode if address.pincode else "NA"

                    cbc_CountrySubentity = ET.SubElement(cac_PostalAddress, "cbc:CountrySubentity")
                    cbc_CountrySubentity.text = address.custom_state_codes if address.custom_state_codes else "NA"
...
def customer_Data(invoice,sales_invoice_doc):
            try:
                ...
                if customer_doc.custom_sst_registration_no:
                    cbc_ID_SST = ET.SubElement(cac_PartyIdentification_1, "cbc:ID")
                    cbc_ID_SST.set("schemeID", "SST")
...
def item_data(invoice, sales_invoice_doc):
    try:
        for single_item in sales_invoice_doc.items:
            ...
            cbc_ItemClassificationCode.text = "001"
            ...
            cbc_TaxCategoryID.text = "01"  # Default to standard tax type
            ...
            # Tax Rate
            cbc_TaxRatePercent = ET.SubElement(cac_TaxSubtotal, "cbc:Percent")
